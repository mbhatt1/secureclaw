...
SecureClaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
secureclaw channels login --channel zalouser
...
```bash
secureclaw message send --channel zalouser --target <threadId> --message "Hello from SecureClaw!"
```
...
```bash
secureclaw channels login --channel zalouser              # Login via QR
secureclaw channels login --channel zalouser --account work
secureclaw channels status --probe
secureclaw channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
secureclaw directory self --channel zalouser
secureclaw directory peers list --channel zalouser --query "name"
secureclaw directory groups list --channel zalouser --query "work"
secureclaw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
secureclaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
secureclaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `secureclaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
secureclaw channels login --channel zalouser --account work
secureclaw message send --channel zalouser --account work --target <id> --message "Hello"
