...
import { approveDevicePairing, listDevicePairing } from "secureclaw/plugin-sdk";
...
function resolveGatewayPort(cfg: SecureClawPluginApi["config"]): number {
  const envRaw =
    process.env.SECURECLAW_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
function resolveScheme(
  cfg: SecureClawPluginApi["config"],
...
async function resolveTailnetHost(api: SecureClawPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: SecureClawPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: SecureClawPluginApi): Promise<ResolveUrlResult> {
