...
// NOTE: This extension is intended to be bundled with SecureClaw.
// When running from source (tests/dev), SecureClaw internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { SecureClawPluginApi } from "../../../src/plugins/types.js";
...
export function createLlmTaskTool(api: SecureClawPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via secureclaw.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-llm-task-"));
