...
`secureclaw.invoke --each`) without adding new SecureClaw code per workflow.
...
This extension depends on SecureClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** SecureClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.secureclaw/extensions` as a standalone plugin directory.
