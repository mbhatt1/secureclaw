/**
 * Twitch channel plugin for SecureClaw.
 *
...
import { buildChannelConfigSchema } from "secureclaw/plugin-sdk";
import type {
 ...
 * for SecureClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: SecureClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: SecureClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: SecureClawConfig): boolean => {
