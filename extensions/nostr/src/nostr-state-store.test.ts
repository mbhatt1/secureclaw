...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.SECURECLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-nostr-"));
  process.env.SECURECLAW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.SECURECLAW_STATE_DIR?.trim() || env.SECURECLAW_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".secureclaw");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.SECURECLAW_STATE_DIR = previous;
    }
