import type { SecureClawPluginApi, SecureClawPluginService } from "secureclaw/plugin-sdk";
...
function normalizeDenyList(cfg: SecureClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: SecureClawPluginApi["config"]): string[] {
...
export default function register(api: SecureClawPluginApi) {
  ...
  const timerService: SecureClawPluginService = {
