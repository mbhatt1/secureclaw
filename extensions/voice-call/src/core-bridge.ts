...
function resolveSecureClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.SECURECLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set SECURECLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveSecureClawRoot(), "dist", "extensionAPI.js");
