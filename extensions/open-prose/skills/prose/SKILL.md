...
metadata: { "secureclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == SecureClaw `sessions_spawn`
- **File I/O** == SecureClaw `read`/`write`
- **Remote fetch** == SecureClaw `web_fetch` (or `exec` with curl when POST is required)
