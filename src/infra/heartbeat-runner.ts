...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: SecureClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: SecureClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: SecureClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: SecureClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: SecureClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: SecureClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: SecureClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
