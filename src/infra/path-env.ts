...
function candidateBinDirs(opts: EnsureSecureClawPathOpts): string[] {
  ...
  // Bundled macOS app: `secureclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "secureclaw");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/secureclaw` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "secureclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `secureclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSecureClawCliOnPath(opts: EnsureSecureClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SECURECLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.SECURECLAW_PATH_BOOTSTRAPPED = "1";
