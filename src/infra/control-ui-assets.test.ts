...
import { resolveSecureClawPackageRoot } from "./secureclaw-root.js";
...
describe("control UI assets helpers", () => {
  it("resolves repo root from src argv1", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-ui-"));
    try {
  ...
  it("resolves control-ui root for dist/gateway bundle argv1", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-ui-"));
    try {
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "secureclaw" }));
  ...
  it("resolves dist control-ui index path from package root argv1", async () => {
    ...
    try {
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "secureclaw" }));
      await fs.writeFile(path.join(tmp, "secureclaw.mjs"), "export {};\n");
      await fs.mkdir(path.join(tmp, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(tmp, "dist", "control-ui", "index.html"), "<html></html>\n");

      expect(await resolveControlUiDistIndexPath(path.join(tmp, "secureclaw.mjs"))).toBe(
  ...
  it("resolves control-ui root for package entrypoint argv1", async () => {
    ...
    try {
      ...
      expect(resolveControlUiRootSync({ argv1: path.join(tmp, "secureclaw.mjs") })).toBe(
        path.join(tmp, "dist", "control-ui"),
      );
    } finally {
      await fs.rm(tmp, { recursive: true, force: true });
  ...
  it("resolves dist control-ui index path from .bin argv1", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(tmp, "node_modules", "secureclaw");
  ...
  it("resolves via fallback when package root resolution fails but package name matches", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-ui-"));
    try {
      // Package named "secureclaw" but resolveSecureClawPackageRoot failed for other reasons
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "secureclaw" }));
      await fs.writeFile(path.join(tmp, "secureclaw.mjs"), "export {};\n");
      await fs.mkdir(path.join(tmp, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(tmp, "dist", "control-ui", "index.html"), "<html></html>\n");
  ...
  it("returns null when package name does not match secureclaw", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-ui-"));
    try {
  ...
  it("resolves control-ui root when argv1 is a symlink (nvm scenario)", async () => {
    ...
    try {
      ...
      const ok = await trySymlink(
        path.join("..", "real-pkg", "secureclaw.mjs"),
        path.join(bin, "secureclaw"),
      );
      if (!ok) {
        return; // symlinks not supported (Windows CI)
      }

      const resolvedRoot = resolveControlUiRootSync({ argv1: path.join(bin, "secureclaw") });
  ...
  it("resolves dist index path via symlinked argv1 (async)", async () => {
    ...
    try {
      ...
      const indexPath = await resolveControlUiDistIndexPath(path.join(bin, "secureclaw"));
