...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.SECURECLAW_MDNS_HOSTNAME = "test-host";
    process.env.SECURECLAW_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("secureclaw-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("secureclaw (SecureClaw)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("secureclaw");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("secureclaw.local");
