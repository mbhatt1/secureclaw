...
export function triggerSecureClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SECURECLAW_SYSTEMD_UNIT,
        process.env.SECURECLAW_PROFILE,
      );
  ...
  const label =
    process.env.SECURECLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SECURECLAW_PROFILE);
