...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SECURECLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*secureclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "secureclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "secureclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "secureclaw-gw");
  ...
  records.push(`_secureclaw-gw._tcp IN PTR ${instanceLabel}._secureclaw-gw._tcp`);
  records.push(`${instanceLabel}._secureclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._secureclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; secureclaw-content-hash: ${contentHash}\n${contentBody}`;
}
