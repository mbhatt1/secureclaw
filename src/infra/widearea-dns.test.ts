...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      domain: "secureclaw.internal.",
      serial: 2025121701,
      gatewayPort: 18789,
      displayName: "Mac Studio (SecureClaw)",
    ...
    expect(txt).toContain(`$ORIGIN secureclaw.internal.`);
    ...
    expect(txt).toContain(`_secureclaw-gw._tcp IN PTR studio-london._secureclaw-gw._tcp`);
    expect(txt).toContain(`studio-london._secureclaw-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (SecureClaw)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/secureclaw`);
  });
