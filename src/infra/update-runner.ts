...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "secureclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("secureclaw doctor", doctorArgv, gitRoot, { SECURECLAW_UPDATE_IN_PROGRESS: "1" }),
    );
