...
import type { SecureClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareSecureClawVersions } from "./version.js";
...
function coerceConfig(value: unknown): SecureClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SecureClawConfig;
}
...
function stampConfigVersion(cfg: SecureClawConfig): SecureClawConfig {
...
function warnIfConfigFromFuture(cfg: SecureClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareSecureClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer SecureClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: SecureClawConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): SecureClawConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as SecureClawConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as SecureClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: SecureClawConfig) {
    clearConfigCache();
...
// module scope. `SECURECLAW_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.SECURECLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.SECURECLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): SecureClawConfig {
...
export async function writeConfigFile(cfg: SecureClawConfig): Promise<void> {
  clearConfigCache();
