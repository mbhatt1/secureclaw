...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real secureclaw.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.SECURECLAW_SESSION_CACHE_TTL_MS;
    process.env.SECURECLAW_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.SECURECLAW_SESSION_CACHE_TTL_MS;
    } else {
      process.env.SECURECLAW_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
