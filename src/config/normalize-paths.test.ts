  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.secureclaw/logs/secureclaw.log" },
        hooks: {
          path: "~/.secureclaw/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.secureclaw/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".secureclaw", "logs", "secureclaw.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".secureclaw", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".secureclaw", "telegram.token"),
      );
