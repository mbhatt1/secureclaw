...
import type { SecureClawConfig, ConfigValidationIssue } from "./types.js";
...
import { SecureClawSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: SecureClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: SecureClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SecureClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as SecureClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as SecureClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as SecureClawConfig,
  };
}
