...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.secureclaw/.env on repeated runtime loads", async () => {
    await withTempHome(async (home) => {
      await withEnvOverride(
        {
          SECURECLAW_STATE_DIR: path.join(home, ".secureclaw"),
          CLAWDBOT_STATE_DIR: undefined,
          SECURECLAW_HOME: undefined,
          CLAWDBOT_HOME: undefined,
          BRAVE_API_KEY: undefined,
          SECURECLAW_DISABLE_CONFIG_CACHE: "1",
        },
        async () => {
