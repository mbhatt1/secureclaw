...
const ETC_SECURECLAW_DIR = path.join(ROOT_DIR, "etc", "secureclaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "secureclaw.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcSecureClawPath(...parts: string[]) {
  return path.join(ETC_SECURECLAW_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcSecureClawPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "secureclaw.json"))).toEqual({
      shared: true,
    });
  });
});
