...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.SECURECLAW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.SECURECLAW_STATE_DIR;
  const configPath = env.SECURECLAW_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    SECURECLAW_PROFILE: profile,
    SECURECLAW_STATE_DIR: stateDir,
    SECURECLAW_CONFIG_PATH: configPath,
    SECURECLAW_GATEWAY_PORT: String(port),
    SECURECLAW_GATEWAY_TOKEN: token,
    SECURECLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    SECURECLAW_SYSTEMD_UNIT: systemdUnit,
    SECURECLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    SECURECLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    SECURECLAW_STATE_DIR: stateDir,
    SECURECLAW_CONFIG_PATH: configPath,
    SECURECLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    SECURECLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    SECURECLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    SECURECLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    SECURECLAW_LOG_PREFIX: "node",
    SECURECLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    SECURECLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    SECURECLAW_SERVICE_VERSION: VERSION,
  };
}
