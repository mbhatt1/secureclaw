...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.SECURECLAW_GATEWAY_PORT).toBe("18789");
    expect(env.SECURECLAW_GATEWAY_TOKEN).toBe("secret");
    expect(env.SECURECLAW_SERVICE_MARKER).toBe("secureclaw");
    expect(env.SECURECLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.SECURECLAW_SERVICE_VERSION).toBe("string");
    expect(env.SECURECLAW_SYSTEMD_UNIT).toBe("secureclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.SECURECLAW_LAUNCHD_LABEL).toBe("ai.secureclaw.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SECURECLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.SECURECLAW_SYSTEMD_UNIT).toBe("secureclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.SECURECLAW_LAUNCHD_LABEL).toBe("ai.secureclaw.work");
    }
  });
});
