...
export const DEFAULT_LOG_DIR = resolvePreferredSecureClawTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "secureclaw.log"); // legacy single-file path

const LOG_PREFIX = "secureclaw";
...
function resolveSettings(): ResolvedSettings {
  let cfg: SecureClawConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => SecureClawConfig;
      };
