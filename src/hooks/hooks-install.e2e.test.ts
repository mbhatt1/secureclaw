...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.SECURECLAW_STATE_DIR;
    prevBundledDir = process.env.SECURECLAW_BUNDLED_HOOKS_DIR;
    process.env.SECURECLAW_STATE_DIR = path.join(baseDir, "state");
    process.env.SECURECLAW_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.SECURECLAW_BUNDLED_HOOKS_DIR;
    } else {
      process.env.SECURECLAW_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
  ...
  it("installs a hook pack and triggers the handler", async () => {
      ...
      JSON.stringify(
        {
          ...
          version: "0.0.0",
          secureclaw: { hooks: ["./hooks/hello-hook"] },
        },
        null,
        2,
      ),
    ...
    await fs.writeFile(
      ...
      [
        ...
        'metadata: {"secureclaw":{"events":["command:new"]}}',
        "---",
        "",
        "# Hello Hook",
        "",
