...
describe("loader", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = path.join(os.tmpdir(), `secureclaw-test-${Date.now()}`);
    await fs.mkdir(tmpDir, { recursive: true });

    // Disable bundled hooks during tests by setting env var to non-existent directory
    originalBundledDir = process.env.SECURECLAW_BUNDLED_HOOKS_DIR;
    process.env.SECURECLAW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      ...
      process.env.SECURECLAW_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
