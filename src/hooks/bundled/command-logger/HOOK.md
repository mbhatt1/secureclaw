...
homepage: https://docs.secureclaw.app/hooks#command-logger
metadata:
  {
    "secureclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.secureclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.secureclaw/logs/commands.log ~/.secureclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/secureclaw`:
   ```
   /home/username/.secureclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
