...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSecureClawAgentDir: vi.fn().mockReturnValue("/tmp/secureclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/secureclaw-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/secureclaw-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSecureClawAgentDir: mocks.resolveSecureClawAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveSecureClawAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/secureclaw-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/secureclaw-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/secureclaw-agent-custom");
