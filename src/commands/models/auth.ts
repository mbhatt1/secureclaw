...
import { readConfigFileSnapshot, type SecureClawConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: SecureClawConfig, model: string): SecureClawConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("secureclaw plugins install")}\`.`,
    );
  }
