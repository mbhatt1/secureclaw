...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveSecureClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("secureclaw models auth setup-token")}\` or \`${formatCliCommand("secureclaw configure")}\`.`
          : `Run \`${formatCliCommand("secureclaw configure")}\` or set an API key env var.`;
