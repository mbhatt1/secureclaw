...
export async function updateConfig(
  mutator: (cfg: SecureClawConfig) => SecureClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: SecureClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: SecureClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("secureclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
