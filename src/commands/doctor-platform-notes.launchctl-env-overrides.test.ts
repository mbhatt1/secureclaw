...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SECURECLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SECURECLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SECURECLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("SECURECLAW_GATEWAY_PASSWORD");
  });
