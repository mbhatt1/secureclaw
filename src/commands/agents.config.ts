...
export function listAgentEntries(cfg: SecureClawConfig): AgentEntry[] {
...
function resolveAgentName(cfg: SecureClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: SecureClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: SecureClawConfig): AgentSummary[] {
