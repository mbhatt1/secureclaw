...
function resolveMode(cfg: SecureClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("SecureClaw doctor");

  const root = await resolveSecureClawPackageRoot({
  ...
  let cfg: SecureClawConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("secureclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("secureclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("secureclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("secureclaw doctor --fix")}" to apply changes.`);
  }
