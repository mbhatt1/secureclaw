...
const SEARCH_TOOL = "https://docs.secureclaw.app/mcp.SearchSecureClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.secureclaw.app");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('secureclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.secureclaw.app/");
      runtime.log(`Search: ${formatCliCommand('secureclaw docs "your query"')}`);
    }
    return;
  }
