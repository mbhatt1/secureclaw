...
function captureEnv(): EnvSnapshot {
  return {
    ...
    disableConfigCache: process.env.SECURECLAW_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("SECURECLAW_STATE_DIR", prev.stateDir);
  restoreEnvVar("SECURECLAW_CONFIG_PATH", prev.configPath);
  restoreEnvVar("SECURECLAW_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("SECURECLAW_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("SECURECLAW_SKIP_CRON", prev.skipCron);
  restoreEnvVar("SECURECLAW_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("SECURECLAW_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("SECURECLAW_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("CUSTOM_API_KEY", prev.customApiKey);
  restoreEnvVar("SECURECLAW_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  const configPath = path.join(tempHome, "secureclaw.json");
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores Z.AI API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("secureclaw-onboard-zai-", async ({ configPath, runtime }) => {
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withOnboardEnv("secureclaw-onboard-zai-cn-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("secureclaw-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("secureclaw-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("secureclaw-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("secureclaw-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("secureclaw-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("secureclaw-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("secureclaw-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "secureclaw-onboard-custom-provider-infer-",
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "secureclaw-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "secureclaw-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("fails custom provider auth when compatibility is invalid", async () => {
    await withOnboardEnv(
      "secureclaw-onboard-custom-provider-invalid-compat-",
      async ({ runtime }) => {
        await expect(
          runNonInteractive(
            {
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("secureclaw-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractive(
          {
            nonInteractive: true,
  ...
  it("fails inferred custom auth when required flags are incomplete", async () => {
    await withOnboardEnv(
      "secureclaw-onboard-custom-provider-missing-required-",
      async ({ runtime }) => {
        await expect(
          runNonInteractive(
            {
