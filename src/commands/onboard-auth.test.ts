...
const requireAgentDir = () => {
  const agentDir = process.env.SECURECLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("SECURECLAW_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.SECURECLAW_STATE_DIR;
  const previousAgentDir = process.env.SECURECLAW_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.SECURECLAW_AGENT_DIR;
    } else {
      process.env.SECURECLAW_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.SECURECLAW_OAUTH_DIR;
  });

  it("writes auth-profiles.json under SECURECLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-oauth-"));
    process.env.SECURECLAW_STATE_DIR = tempStateDir;
    process.env.SECURECLAW_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SECURECLAW_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to SECURECLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-minimax-"));
    process.env.SECURECLAW_STATE_DIR = tempStateDir;
    process.env.SECURECLAW_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SECURECLAW_AGENT_DIR;
