...
beforeEach(() => {
  ...
  resolveSecureClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SECURECLAW_STATE_DIR;
  originalUpdateInProgress = process.env.SECURECLAW_UPDATE_IN_PROGRESS;
  process.env.SECURECLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "secureclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SECURECLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SECURECLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.SECURECLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveSecureClawPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadSecureClawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/secureclaw.json",
    createConfigIO,
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "secureclaw-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.SECURECLAW_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when SECURECLAW_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/secureclaw.json",
      exists: true,
      raw: "{}",
    ...
    process.env.SECURECLAW_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
