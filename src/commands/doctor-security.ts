...
import type { SecureClawConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: SecureClawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("secureclaw security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("secureclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("secureclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("secureclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("secureclaw config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
