...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-onboard-"));
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "secureclaw");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.SECURECLAW_STATE_DIR = stateDir;
    process.env.SECURECLAW_CONFIG_PATH = path.join(stateDir, "secureclaw.json");
