...
const BUNDLED_VERSION =
  (typeof __SECURECLAW_VERSION__ === "string" && __SECURECLAW_VERSION__) ||
  process.env.SECURECLAW_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `secureclaw --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.SECURECLAW_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.SECURECLAW_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[secureclaw] Gateway daemon failed:",
