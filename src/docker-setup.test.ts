...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "secureclaw-docker-setup-"));
...
function createEnv(
  ...
  return {
    ...
    SECURECLAW_GATEWAY_TOKEN: "test-token",
    SECURECLAW_CONFIG_DIR: join(sandbox.rootDir, "config"),
    SECURECLAW_WORKSPACE_DIR: join(sandbox.rootDir, "secureclaw"),
    ...overrides,
  };
}
...
describe("docker-setup.sh", () => {
  it("handles unset optional env vars under strict mode", async () => {
    ...
    const env = createEnv(sandbox, {
      SECURECLAW_DOCKER_APT_PACKAGES: undefined,
      SECURECLAW_EXTRA_MOUNTS: undefined,
      SECURECLAW_HOME_VOLUME: undefined,
    });
    ...
    expect(envFile).toContain("SECURECLAW_DOCKER_APT_PACKAGES=");
    expect(envFile).toContain("SECURECLAW_EXTRA_MOUNTS=");
    expect(envFile).toContain("SECURECLAW_HOME_VOLUME=");
  });
  ...
  it("supports a home volume when extra mounts are empty", async () => {
    ...
    const env = createEnv(sandbox, {
      ...
      SECURECLAW_HOME_VOLUME: "secureclaw-home",
    });
    ...
    expect(extraCompose).toContain("secureclaw-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("secureclaw-home:");
  });
  ...
  it("plumbs SECURECLAW_DOCKER_APT_PACKAGES into .env and docker build args", async () => {
    const sandbox = await createDockerSetupSandbox();
    const env = createEnv(sandbox, {
      SECURECLAW_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      SECURECLAW_EXTRA_MOUNTS: "",
      SECURECLAW_HOME_VOLUME: "",
    });
    ...
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("SECURECLAW_DOCKER_APT_PACKAGES=ffmpeg build-essential");

    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg SECURECLAW_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
