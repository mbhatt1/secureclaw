...
export function resolveConfigDir(
  ...
  const override = env.SECURECLAW_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.SECURECLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$SECURECLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via SECURECLAW_STATE_DIR.
