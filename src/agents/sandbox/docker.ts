...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "secureclaw.sandbox=1");
  args.push("--label", `secureclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `secureclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `secureclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("secureclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`secureclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`secureclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("secureclaw sandbox recreate --all");
}
