...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveSecureClawAgentDir();
  await ensureSecureClawModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createSecureClawCodingTools({
      exec: {
    ...
    const docsPath = await resolveSecureClawDocsPath({
      workspaceDir: effectiveWorkspace,
