...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.SECURECLAW_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-test-"));
    process.env.SECURECLAW_STATE_DIR = stateDir;
    process.env.SECURECLAW_PROFILE = "isolated";

    try {
      const tool = createSecureClawTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: secureclaw --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/secureclaw" } }\n}\n';
