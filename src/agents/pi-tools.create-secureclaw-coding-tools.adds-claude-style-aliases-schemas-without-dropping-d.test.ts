...
describe("createSecureClawCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from secureclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "secureclaw-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "secureclaw-workspace"),
    ...
    const tools = createSecureClawCodingTools({ sandbox });
