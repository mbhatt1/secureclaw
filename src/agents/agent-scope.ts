...
type AgentEntry = NonNullable<NonNullable<SecureClawConfig["agents"]>["list"]>[number];
...
function listAgents(cfg: SecureClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: SecureClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: SecureClawConfig): string {
  const agents = listAgents(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: SecureClawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentWorkspaceDir(cfg: SecureClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: SecureClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
