...
type WebSearchConfig = NonNullable<SecureClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: SecureClawConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("secureclaw configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.secureclaw.app/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://secureclaw.app",
      "X-Title": "SecureClaw Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
