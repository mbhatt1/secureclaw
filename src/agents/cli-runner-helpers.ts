...
/**
 * Cleanup suspended SecureClaw CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: SecureClawConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-cli-images-"));
  const paths: string[] = [];
