...
describe("canvas host state dir defaults", () => {
  ...
  it("uses SECURECLAW_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("SecureClaw Canvas");
    } finally {
      await handler.close();
