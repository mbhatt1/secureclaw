...
export function isCommandEnabled(cfg: SecureClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: SecureClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as SecureClawConfig),
...
export function getCommandDetection(_cfg?: SecureClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: SecureClawConfig) {
