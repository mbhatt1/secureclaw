...
const STEP_LABELS: Record<string, string> = {
  ...
  "secureclaw doctor entry": "Checking doctor entrypoint",
  "secureclaw doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "secureclaw";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const SECURECLAW_REPO_URL = "https://github.com/mbhatt1/secureclaw.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("secureclaw@")) {
    return trimmed.slice("secureclaw@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "secureclaw.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("secureclaw completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.SECURECLAW_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", SECURECLAW_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `SECURECLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set SECURECLAW_GIT_DIR to an empty folder or an secureclaw checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`SECURECLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveSecureClawPackageRoot({
  ...
  defaultRuntime.log(theme.heading("SecureClaw update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating SecureClaw..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this SecureClaw install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("secureclaw doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("secureclaw gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g secureclaw@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g secureclaw@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.SECURECLAW_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.SECURECLAW_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("secureclaw gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("secureclaw doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("secureclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("secureclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `secureclaw update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `SECURECLAW_GIT_DIR points at a non-git directory: ${gitDir}. Set SECURECLAW_GIT_DIR to an empty folder or an secureclaw checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via SECURECLAW_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update SecureClaw to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["secureclaw update", "Update a source checkout (git)"],
        ["secureclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["secureclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["secureclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["secureclaw update --no-restart", "Update without restarting the service"],
        ["secureclaw update --json", "Output result as JSON"],
        ["secureclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["secureclaw update wizard", "Interactive update wizard"],
        ["secureclaw --update", "Shorthand for secureclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run secureclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.secureclaw.app/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["secureclaw update status", "Show channel + version status."],
          ["secureclaw update status --json", "JSON output."],
          ["secureclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.secureclaw.app/cli/update")}`,
    )
    .action(async (opts) => {
      try {
