...
async function runGatewayCommand(opts: GatewayRunOpts) {
  const isDevProfile = process.env.SECURECLAW_PROFILE?.trim().toLowerCase() === "dev";
  ...
  if (opts.claudeCliLogs) {
    ...
    process.env.SECURECLAW_CLAUDE_CLI_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.SECURECLAW_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.SECURECLAW_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (!opts.allowUnconfigured && mode !== "local") {
    if (!configExists) {
      defaultRuntime.error(
        `Missing config. Run \`${formatCliCommand("secureclaw setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
      );
    } else {
      defaultRuntime.error(
  ...
  if (resolvedAuthMode === "token" && !hasToken && !resolvedAuth.allowTailscale) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token (or SECURECLAW_GATEWAY_TOKEN), or pass --token.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (resolvedAuthMode === "password" && !hasPassword) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or SECURECLAW_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (bind !== "loopback" && !hasSharedSecret) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.token/password (or SECURECLAW_GATEWAY_TOKEN/SECURECLAW_GATEWAY_PASSWORD) or pass --token/--password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  try {
    ...
    if (
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("secureclaw gateway stop")}`,
      );
      try {
...
export function addGatewayRunCommand(cmd: Command): Command {
  return cmd
    ...
    .option(
      "--token <token>",
      "Shared token required in connect.params.auth.token (default: SECURECLAW_GATEWAY_TOKEN env if set)",
    )
