...
${formatHelpExamples([
  ['secureclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['secureclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'secureclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'secureclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['secureclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'secureclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.secureclaw.app/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.secureclaw.app/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "secureclaw agents set-identity --identity-file ~/.secureclaw/workspace/IDENTITY.md --agent main",
