...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# SecureClaw Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(secureclaw completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# SecureClaw Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(secureclaw completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $SECURECLAW_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "secureclaw") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c secureclaw -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'secureclaw gateway' and not 'secureclaw other gateway', we need to check the sequence.
