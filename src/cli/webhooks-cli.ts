...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.secureclaw.app/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + SecureClaw hooks")
    ...
    .option("--hook-url <url>", "SecureClaw hook URL")
    .option("--hook-token <token>", "SecureClaw hook token")
