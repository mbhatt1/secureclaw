  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your SecureClaw state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your SecureClaw state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  if (chatCompletionsEnabled || responsesEnabled) {
    ...
    findings.push({
      ...
      detail:
        `${enabledEndpoints.join(", ")} support x-secureclaw-session-key. ` +
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("secureclaw security audit --deep")}".`,
    });
    return findings;
  }
...
function collectLoggingFindings(cfg: SecureClawConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: SecureClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
    ...
    const token =
      mode === "remote"
        ? typeof remote?.token === "string" && remote.token.trim()
          ? remote.token.trim()
          : undefined
        : process.env.SECURECLAW_GATEWAY_TOKEN?.trim() ||
          (typeof authToken === "string" && authToken.trim() ? authToken.trim() : undefined);
    const password =
      process.env.SECURECLAW_GATEWAY_PASSWORD?.trim() ||
      (mode === "remote"
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("secureclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("secureclaw security audit --deep")}".`,
    });
  }
