...
function setSlackDmPolicy(cfg: SecureClawConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "SecureClaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for SecureClaw`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: SecureClawConfig, allowFrom: string[]): SecureClawConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
