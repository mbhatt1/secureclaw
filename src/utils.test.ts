...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "secureclaw-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "secureclaw-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "secureclaw-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "secureclaw-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.secureclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "secureclaw-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers SECURECLAW_HOME over HOME", () => {
    vi.stubEnv("SECURECLAW_HOME", "/srv/secureclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/secureclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $SECURECLAW_HOME prefix when SECURECLAW_HOME is set", () => {
    vi.stubEnv("SECURECLAW_HOME", "/srv/secureclaw-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("shortenHomeInString", () => {
  it("uses $SECURECLAW_HOME replacement when SECURECLAW_HOME is set", () => {
    vi.stubEnv("SECURECLAW_HOME", "/srv/secureclaw-home");
    vi.stubEnv("HOME", "/home/other");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/secureclaw")).toBe(path.resolve(os.homedir(), "secureclaw"));
  });
  ...
  it("prefers SECURECLAW_HOME for tilde expansion", () => {
    vi.stubEnv("SECURECLAW_HOME", "/srv/secureclaw-home");
    vi.stubEnv("HOME", "/home/other");
