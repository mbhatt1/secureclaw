...
import { discoverSecureClawPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SecureClawPluginDefinition;
  register?: SecureClawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as SecureClawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as SecureClawPluginDefinition;
...
export function loadSecureClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverSecureClawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "secureclaw/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: SecureClawPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as SecureClawPluginModule;
    } catch (err) {
