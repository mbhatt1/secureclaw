...
import { loadSecureClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.SECURECLAW_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `secureclaw-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "secureclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.SECURECLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadSecureClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.SECURECLAW_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadSecureClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadSecureClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
