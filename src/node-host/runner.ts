...
import { ensureSecureClawCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.SECURECLAW_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.SECURECLAW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureSecureClawCliOnPath({ pathEnv: process.env.PATH ?? "" });
