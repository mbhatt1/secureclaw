...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.SECURECLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.SECURECLAW_SKIP_GMAIL_WATCHER;
    process.env.SECURECLAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.SECURECLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.SECURECLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SECURECLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
