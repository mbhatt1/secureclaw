...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/secureclaw/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (SecureClaw)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.SECURECLAW_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "secureclaw");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.SECURECLAW_TAILNET_DNS?.trim();
