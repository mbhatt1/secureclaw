...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.SECURECLAW_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.SECURECLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.SECURECLAW_GATEWAY_TOKEN = gatewayToken;
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.SECURECLAW_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createSecureClawTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
