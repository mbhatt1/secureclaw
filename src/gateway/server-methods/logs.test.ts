...
describe("logs.tail", () => {
  ...
  it("falls back to latest rolling log file when today is missing", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-logs-"));
    const older = path.join(tempDir, "secureclaw-2026-01-20.log");
    const newer = path.join(tempDir, "secureclaw-2026-01-21.log");
    ...
    setLoggerOverride({ file: path.join(tempDir, "secureclaw-2026-01-22.log") });
