...
describe("gateway server chat", () => {
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-gw-"));
  ...
  test("routes chat.send slash commands without agent runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "secureclaw-gw-"));
    try {
