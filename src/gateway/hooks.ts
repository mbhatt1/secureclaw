...
export function resolveHooksConfig(cfg: SecureClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SecureClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-secureclaw-token"] === "string"
      ? req.headers["x-secureclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
