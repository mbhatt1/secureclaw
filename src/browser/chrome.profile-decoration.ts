...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".secureclaw-profile-decorated");
}
 ...
 */
export function decorateSecureClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SECURECLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SECURECLAW_BROWSER_COLOR).toUpperCase();
