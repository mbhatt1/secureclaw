import type { BrowserConfig, BrowserProfileConfig, SecureClawConfig } from "../config/config.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "secureclaw" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_SECURECLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "secureclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_SECURECLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_SECURECLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an SecureClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for secureclaw/secureclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: SecureClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_SECURECLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "secureclaw";
