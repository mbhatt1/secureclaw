...
vi.mock("./chrome.js", () => ({
  resolveSecureClawUserDataDir: vi.fn(() => "/tmp/secureclaw-test/secureclaw/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveSecureClawUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "secureclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveSecureClawUserDataDir).mockReturnValue(userDataDir);
