...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.secureclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.secureclaw/node.json` (node id + pairing token).
...
Path: `~/.secureclaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.secureclaw/exec-approvals.sock",
...
- Unix socket at `~/.secureclaw/exec-approvals.sock` (0600).
