...
- Publish as `secureclaw/plugin-sdk` (or export from core under `secureclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `SecureClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SecureClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SecureClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SecureClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SecureClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SecureClawConfig): string;
  };
};
```
...
- Introduce `secureclaw/plugin-sdk`.
- Add `api.runtime` to `SecureClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `secureclawRuntime: ">=2026.2.0"`).
