...
- **Media understanding (audio)**: If audio understanding is enabled (or auto‑detected), SecureClaw:
...
SecureClaw auto-detects in this order and stops at the first working option:
...
When `requireMention: true` is set for a group chat, SecureClaw now transcribes audio **before** checking for mentions. This allows voice notes to be processed even when they contain mentions.

**How it works:**

1. If a voice message has no text body and the group requires mentions, SecureClaw performs a "preflight" transcription.
