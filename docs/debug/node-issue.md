...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in SecureClaw
...
Running SecureClaw via Node with `tsx` fails at startup with:

```
[secureclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `secureclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch secureclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node secureclaw.mjs status` works on Node 25.
