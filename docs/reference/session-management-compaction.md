...
This document explains how SecureClaw manages sessions end-to-end:
...
SecureClaw is designed around a single **Gateway process** that owns session state.
...
SecureClaw persists sessions in two layers:
...
- Transcripts: `~/.secureclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

SecureClaw resolves these via `src/config/sessions.ts`.
...
SecureClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (SecureClaw consumes the events, but Pi decides when to compact).
...
SecureClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, SecureClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, SecureClaw leaves it alone.
...
- `secureclaw status` (CLI)
- `secureclaw sessions` / `sessions --json`
...
SecureClaw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- SecureClaw strips/suppresses this in the delivery layer.

As of `2026.1.10`, SecureClaw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
SecureClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but SecureClaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `secureclaw status`.
