...
SecureClaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- SecureClaw owns lifecycle when `channels.signal.autoStart=true`.
...
- SecureClaw spawns `imsg rpc` as a child process (legacy iMessage integration).
