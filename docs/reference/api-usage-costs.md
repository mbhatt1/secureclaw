...
SecureClaw features that can generate provider usage or paid API calls.
...
- `secureclaw status --usage` and `secureclaw channels list` show provider **usage windows**
...
SecureClaw can pick up credentials from:
...
- `secureclaw status --usage`
- `secureclaw models status --json`
...
`secureclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
