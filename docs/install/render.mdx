...
Deploy SecureClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/mbhatt1/secureclaw)
...
repository configures everything needed to run SecureClaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: SECURECLAW_STATE_DIR
        value: /data/.secureclaw
      - key: SECURECLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: SECURECLAW_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: secureclaw-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/secureclaw`.
...
If you use the original SecureClaw repository, Render will not auto-deploy your SecureClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For SecureClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any SecureClaw host.
