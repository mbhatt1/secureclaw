---
summary: "SecureClaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up SecureClaw on DigitalOcean
  - Looking for cheap VPS hosting for SecureClaw
title: "DigitalOcean"
---

# SecureClaw on DigitalOcean

## Goal

Run a persistent SecureClaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status secureclaw-gateway.service

# View logs
journalctl --user -u secureclaw-gateway.service -f
```
...
```bash
secureclaw channels login whatsapp
# Scan QR code
```
...
- `~/.secureclaw/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u secureclaw --no-pager -n 50
```
