...
- Same path: run the app from a fixed location (for SecureClaw, `dist/SecureClaw.app`).
...
Workaround: move files into the SecureClaw workspace (`~/.secureclaw/workspace`) if you want to avoid per-folder grants.
