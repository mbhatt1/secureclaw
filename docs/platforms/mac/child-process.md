...
service via the external `secureclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`SECURECLAW_PROFILE`; legacy `com.secureclaw.*` is supported).
