---
summary: "SecureClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up SecureClaw on a Raspberry Pi
  - Running SecureClaw on ARM devices
...
Most SecureClaw features work on ARM64, but some external binaries may need ARM builds:
...
# Common fix: rebuild
cd ~/secureclaw  # if using hackable install
npm run build
sudo systemctl restart secureclaw
```
