...
Tlon is a decentralized messenger built on Urbit. SecureClaw connects to your Urbit ship and can
...
```bash
secureclaw plugins install @secureclaw/tlon
```

Local checkout (when running from a git repo):

```bash
secureclaw plugins install ./extensions/tlon
```
...
Use these with `secureclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, SecureClaw replies in-thread.
