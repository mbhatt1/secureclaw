...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects SecureClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
secureclaw plugins install @secureclaw/feishu
```

Local checkout (when running from a git repo):

```bash
secureclaw plugins install ./extensions/feishu
```
...
If you just installed SecureClaw, run the wizard:
...
2. Configuring app credentials in SecureClaw
...
1. You already ran `secureclaw channels add` for Feishu
2. The gateway is running (`secureclaw gateway status`)
...
Edit `~/.secureclaw/secureclaw.json`:
...
```bash
secureclaw pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  secureclaw pairing list feishu
  secureclaw pairing approve feishu <CODE>
  ```
...
2. Run `secureclaw logs --follow` and look for `chat_id`
...
2. Run `secureclaw logs --follow` and look for `open_id`
...
| `secureclaw gateway restart` | Restart gateway service       |
| `secureclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `secureclaw logs --follow`
...
5. Ensure the gateway is running: `secureclaw gateway status`
6. Check logs: `secureclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.secureclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.secureclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
