...
```
    ...
    - or read `chat.id` from `secureclaw logs --follow`
...
- DM messages can carry `message_thread_id`; SecureClaw routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Draft streaming in Telegram DMs">
    SecureClaw can stream partial replies with Telegram draft bubbles (`sendMessageDraft`).
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, SecureClaw retries as plain text.
...
```
    ...
    - `~/.secureclaw/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, SecureClaw enqueues system events like:
...
```bash
secureclaw message send --channel telegram --target 123456789 --message "hi"
secureclaw message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `secureclaw channels status` warns when config expects unmentioned group messages.
    - `secureclaw channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `secureclaw logs --follow` for skip reasons
