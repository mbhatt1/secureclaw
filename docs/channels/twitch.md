...
read_when:
  - Setting up Twitch chat integration for SecureClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. SecureClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
secureclaw plugins install @secureclaw/twitch
```

Local checkout (when running from a git repo):

```bash
secureclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SECURECLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "secureclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SECURECLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
SECURECLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
