...
Matrix is an open, decentralized messaging protocol. SecureClaw connects as a Matrix **user**
...
```bash
secureclaw plugins install @secureclaw/matrix
```

Local checkout (when running from a git repo):

```bash
secureclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `secureclaw plugins install @secureclaw/matrix`
   - From a local checkout: `secureclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: SecureClaw calls the same
     login endpoint, stores the access token in `~/.secureclaw/credentials/matrix/credentials.json`,
...
- On first connection, SecureClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  SecureClaw logs a warning.
...
`~/.secureclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `secureclaw pairing list matrix`
  - `secureclaw pairing approve matrix <CODE>`
...
- On startup, SecureClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
