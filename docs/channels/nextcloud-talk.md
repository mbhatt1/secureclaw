...
```bash
secureclaw plugins install @secureclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
secureclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "SecureClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `secureclaw pairing list nextcloud-talk`
  - `secureclaw pairing approve nextcloud-talk <CODE>`
