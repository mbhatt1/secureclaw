...
- Jobs persist under `~/.secureclaw/cron/` so restarts don’t lose schedules.
...
secureclaw cron list
secureclaw cron run <job-id>
secureclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.secureclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `secureclaw cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, SecureClaw defaults to `announce`.
...
- Job store: `~/.secureclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.secureclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.secureclaw/cron/jobs.json",
...
secureclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
secureclaw cron edit <jobId> --agent ops
secureclaw cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
secureclaw cron run <jobId>
secureclaw cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
secureclaw cron edit <jobId> \
...
```bash
secureclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`secureclaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SECURECLAW_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- SecureClaw applies exponential retry backoff for recurring jobs after consecutive errors:
