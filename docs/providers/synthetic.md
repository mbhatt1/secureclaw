---
summary: "Use Synthetic's Anthropic-compatible API in SecureClaw"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. SecureClaw registers it as the
...
Note: SecureClaw's Anthropic client appends `/v1` to the base URL, so use
