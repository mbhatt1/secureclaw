---
summary: "Use Venice AI privacy-focused models in SecureClaw"
read_when:
  - You want privacy-focused inference in SecureClaw
...
```bash
secureclaw onboard --auth-choice venice-api-key
```
...
```bash
secureclaw chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, SecureClaw shows all available Venice models. Pick based on your needs:
...
```bash
secureclaw models set venice/claude-opus-45
secureclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
secureclaw models list | grep venice
```

## Configure via `secureclaw configure`

1. Run `secureclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
SecureClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
secureclaw chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
secureclaw chat --model venice/claude-opus-45

# Use uncensored model
secureclaw chat --model venice/venice-uncensored

# Use vision model with image
secureclaw chat --model venice/qwen3-vl-235b-a22b

# Use coding model
secureclaw chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `secureclaw models list` to see currently available models. Some models may be temporarily offline.
