---
summary: "Use MiniMax M2.1 in SecureClaw"
read_when:
  - You want MiniMax models in SecureClaw
...
```bash
secureclaw plugins enable minimax-portal-auth  # skip if already loaded.
secureclaw gateway restart  # restart if gateway is already running
secureclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/mbhatt1/secureclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `secureclaw.json`:
...
- Use `secureclaw models list` and `secureclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `secureclaw configure` and selecting **MiniMax M2.1**, or
