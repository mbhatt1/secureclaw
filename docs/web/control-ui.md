...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/secureclaw`)
...
If the page fails to load, start the Gateway first: `secureclaw gateway`.
...
you revoke it with `secureclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.secureclaw/secureclaw.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. SecureClaw
...
SecureClaw **blocks** Control UI connections without device identity.
