...
```bash
secureclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `secureclaw tui --deliver`
...
- Check the Gateway logs: `secureclaw logs --follow`.
- Confirm the agent can run: `secureclaw status` and `secureclaw models status`.
...
- No agents in picker: check `secureclaw agents list` and your routing config.
