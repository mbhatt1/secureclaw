...
当你使用 `--channel` **显式**切换渠道时，SecureClaw 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/secureclaw`，可通过 `SECURECLAW_GIT_DIR` 覆盖），
...
当你使用 `secureclaw update` 切换渠道时，SecureClaw 还会同步插件来源：
