...
read_when:
  ...
  - 你想要所有内容都固定并以声明式管理
summary: 使用 Nix 声明式安装 SecureClaw
title: Nix
x-i18n:
...
使用 Nix 运行 SecureClaw 的推荐方式是通过 **[nix-secureclaw](https://github.com/secureclaw/nix-secureclaw)** — 一个开箱即用的 Home Manager 模块。
...
> **📦 完整指南：[github.com/secureclaw/nix-secureclaw](https://github.com/secureclaw/nix-secureclaw)**
>
> nix-secureclaw 仓库是 Nix 安装的权威来源。本页只是一个快速概述。
...
当设置 `SECURECLAW_NIX_MODE=1` 时（nix-secureclaw 会自动设置）：

SecureClaw 支持 **Nix 模式**，使配置确定性并禁用自动安装流程。
通过导出以下环境变量启用：
...
SecureClaw 从 `SECURECLAW_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `SECURECLAW_STATE_DIR` 中。

- `SECURECLAW_STATE_DIR`（默认：`~/.secureclaw`）
- `SECURECLAW_CONFIG_PATH`（默认：`$SECURECLAW_STATE_DIR/secureclaw.json`）
...
[`scripts/package-mac-app.sh`](https://github.com/mbhatt1/secureclaw/blob/main/scripts/package-mac-app.sh) 将此模板复制到应用包中并修补动态字段（bundle ID、版本/构建号、Git SHA、Sparkle 密钥）。这使 plist 对于 SwiftPM 打包和 Nix 构建保持确定性（它们不依赖完整的 Xcode 工具链）。

## 相关内容

- [nix-secureclaw](https://github.com/secureclaw/nix-secureclaw) — 完整设置指南
