...
read_when:
  - 安装 SecureClaw
  - 你想从 GitHub 安装
summary: 安装 SecureClaw（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
通过 npm 全局安装 `secureclaw` 并运行新手引导。
...
pnpm approve-builds -g                # 批准 secureclaw、node-llama-cpp、sharp 等
pnpm add -g secureclaw@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm secureclaw ...` 运行仓库命令。
...
- 运行新手引导：`secureclaw onboard --install-daemon`
- 快速检查：`secureclaw doctor`
- 检查 Gateway 网关健康状态：`secureclaw status` + `secureclaw health`
- 打开仪表板：`secureclaw dashboard`
...
```bash
# 显式 npm
curl -fsSL https://secureclaw.app/install.sh | bash -s -- --install-method npm
...
- `SECURECLAW_GIT_DIR=...`
- `SECURECLAW_GIT_UPDATE=0|1`
- `SECURECLAW_NO_PROMPT=1`
- `SECURECLAW_DRY_RUN=1`
- `SECURECLAW_NO_ONBOARD=1`
- `SHARP_IGNORE_GLOBAL_LIBVIPS=0|1`（默认：`1`；避免 `sharp` 针对系统 libvips 构建）

## 故障排除：找不到 `secureclaw`（PATH）
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `secureclaw`）。
