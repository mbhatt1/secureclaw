...
你可以在 `~/.secureclaw/secureclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.secureclaw/secureclaw.json` 的 `logging` 下。
...
- SecureClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `secureclaw plugins enable diagnostics-otel` 启用插件。
...
- `secureclaw.tokens`（计数器，属性：`secureclaw.token`、`secureclaw.channel`、`secureclaw.provider`、`secureclaw.model`）
- `secureclaw.cost.usd`（计数器，属性：`secureclaw.channel`、`secureclaw.provider`、`secureclaw.model`）
- `secureclaw.run.duration_ms`（直方图，属性：`secureclaw.channel`、`secureclaw.provider`、`secureclaw.model`）
- `secureclaw.context.tokens`（直方图，属性：`secureclaw.context`、`secureclaw.channel`、`secureclaw.provider`、`secureclaw.model`）

消息流：

- `secureclaw.webhook.received`（计数器，属性：`secureclaw.channel`、`secureclaw.webhook`）
- `secureclaw.webhook.error`（计数器，属性：`secureclaw.channel`、`secureclaw.webhook`）
- `secureclaw.webhook.duration_ms`（直方图，属性：`secureclaw.channel`、`secureclaw.webhook`）
- `secureclaw.message.queued`（计数器，属性：`secureclaw.channel`、`secureclaw.source`）
- `secureclaw.message.processed`（计数器，属性：`secureclaw.channel`、`secureclaw.outcome`）
- `secureclaw.message.duration_ms`（直方图，属性：`secureclaw.channel`、`secureclaw.outcome`）

队列 + 会话：

- `secureclaw.queue.lane.enqueue`（计数器，属性：`secureclaw.lane`）
- `secureclaw.queue.lane.dequeue`（计数器，属性：`secureclaw.lane`）
- `secureclaw.queue.depth`（直方图，属性：`secureclaw.lane` 或 `secureclaw.channel=heartbeat`）
- `secureclaw.queue.wait_ms`（直方图，属性：`secureclaw.lane`）
- `secureclaw.session.state`（计数器，属性：`secureclaw.state`、`secureclaw.reason`）
- `secureclaw.session.stuck`（计数器，属性：`secureclaw.state`）
- `secureclaw.session.stuck_age_ms`（直方图，属性：`secureclaw.state`）
- `secureclaw.run.attempt`（计数器，属性：`secureclaw.attempt`）
...
- `secureclaw.webhook.error`
  - `secureclaw.channel`、`secureclaw.webhook`、`secureclaw.chatId`、`secureclaw.error`
- `secureclaw.message.processed`
  - `secureclaw.channel`、`secureclaw.outcome`、`secureclaw.chatId`、`secureclaw.messageId`、`secureclaw.sessionKey`、`secureclaw.sessionId`、`secureclaw.reason`
- `secureclaw.session.stuck`
  - `secureclaw.state`、`secureclaw.ageMs`、`secureclaw.queueDepth`、`secureclaw.sessionKey`、`secureclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `secureclaw doctor`。
