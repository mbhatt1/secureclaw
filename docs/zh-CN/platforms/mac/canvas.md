...
- `~/Library/Application Support/SecureClaw/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `secureclaw-canvas://<session>/<path>`

示例：

- `secureclaw-canvas://main/` → `<canvasRoot>/main/index.html`
- `secureclaw-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `secureclaw-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
secureclaw nodes canvas present --node <id>
secureclaw nodes canvas navigate --node <id> --url "/"
secureclaw nodes canvas eval --node <id> --js "document.title"
secureclaw nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__secureclaw__/a2ui/
```
...
secureclaw nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
secureclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "secureclaw://agent?message=Review%20this%20design";
```
