...
Gateway 网关；如果无法访问，它会通过外部 `secureclaw` CLI（无嵌入式运行时）启用 launchd
...
- 应用安装标记为 `bot.molt.gateway` 的按用户 LaunchAgent
  （使用 `--profile`/`SECURECLAW_PROFILE` 时为 `bot.molt.<profile>`；支持旧版 `com.secureclaw.*`）。
...
- 写入 `~/.secureclaw/disable-launchagent`。
...
`--attach-only`（或 `--no-launchd`）启动它。这会设置 `~/.secureclaw/disable-launchagent`，
