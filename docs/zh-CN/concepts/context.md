...
"上下文"是 **SecureClaw 在一次运行中发送给模型的所有内容**。它受模型的**上下文窗口**（token 限制）约束。

新手心智模型：

- **系统提示词**（SecureClaw 构建）：规则、工具、Skills 列表、时间/运行时，以及注入的工作区文件。
...
## SecureClaw 如何构建系统提示词

系统提示词由 **SecureClaw 拥有**，每次运行时重建。它包括：
...
默认情况下，SecureClaw 注入一组固定的工作区文件（如果存在）：
