...
LINE 通过 LINE Messaging API 连接到 SecureClaw。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
secureclaw plugins install @secureclaw/line
```

本地检出（从 git 仓库运行时）：

```bash
secureclaw plugins install ./extensions/line
```
...
```bash
secureclaw pairing list line
secureclaw pairing approve line <CODE>
```
