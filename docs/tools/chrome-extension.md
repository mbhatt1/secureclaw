---
summary: "Chrome extension: let SecureClaw drive your existing Chrome tab"
read_when:
...
The SecureClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate secureclaw-managed Chrome profile.
...
SecureClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the SecureClaw release (npm package) as static files. There is no separate “build” step.

After upgrading SecureClaw:

- Re-run `secureclaw browser extension install` to refresh the installed files under your SecureClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

SecureClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want SecureClaw to control.
...
- `ON`: attached; SecureClaw can drive that tab.
...
Debugging: `secureclaw sandbox explain`
...
`secureclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `secureclaw browser extension install` first to copy the extension to a stable location under your SecureClaw state directory.
...
- **This is not isolated** like the dedicated secureclaw-managed profile.
