...
- **gateway host** → `secureclaw` process on the gateway machine
...
`~/.secureclaw/exec-approvals.json` directly.

CLI: `secureclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
