---
summary: "Agent tool surface for SecureClaw (browser, canvas, nodes, message, cron) replacing legacy `secureclaw-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `secureclaw-*` skills
title: "Tools"
---

# Tools (SecureClaw)

SecureClaw exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `secureclaw-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `secureclaw.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, SecureClaw logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:secureclaw`: all built-in SecureClaw tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`secureclaw node run`).
...
- Requires a Brave API key (recommended: `secureclaw configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated SecureClaw-managed browser.
...
- Quick smoke: `secureclaw nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `secureclaw gateway` restart in-place)
