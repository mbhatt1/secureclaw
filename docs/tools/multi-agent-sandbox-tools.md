...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `secureclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `secureclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=secureclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${SECURECLAW_STATE_DIR:-$HOME/.secureclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
