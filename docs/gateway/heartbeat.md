...
- During heartbeat runs, SecureClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `secureclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, SecureClaw skips the heartbeat run entirely (no model call).
...
```bash
secureclaw system event --text "Check for urgent follow-ups" --mode now
```
