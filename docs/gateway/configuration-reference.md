...
description: "Complete field-by-field reference for ~/.secureclaw/secureclaw.json"
---

# Configuration Reference

Every field available in `~/.secureclaw/secureclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — SecureClaw uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `secureclaw doctor` into `whatsapp/default`.
...
Mattermost ships as a plugin: `secureclaw plugins install @secureclaw/mattermost`.
...
SecureClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@secureclaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `secureclaw.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, SecureClaw auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, SecureClaw assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.secureclaw/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.secureclaw/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@secureclaw"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:secureclaw` | All built-in tools (excludes provider plugins)                                           |
...
SecureClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.secureclaw/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `SECURECLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `secureclaw onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `secureclaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `secureclaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `secureclaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `secureclaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `secureclaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.secureclaw/extensions`, `<workspace>/.secureclaw/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or SECURECLAW_GATEWAY_PASSWORD
      allowTailscale: true,
    },
    tailscale: {
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `SECURECLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.secureclaw-dev` + port `19001`), `--profile <name>` (uses `~/.secureclaw-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-secureclaw-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `SECURECLAW_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    port: 18793,
    liveReload: true,
    // enabled: false, // or SECURECLAW_SKIP_CANVAS_HOST=1
  },
}
```
...
- Also serves A2UI at `/__secureclaw__/a2ui/`.
...
- Hostname defaults to `secureclaw`. Override with `SECURECLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.secureclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `secureclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.secureclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${SECURECLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.secureclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/secureclaw/secureclaw-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `secureclaw doctor --fix` can strip unknown keys).
