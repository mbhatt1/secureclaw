...
read_when:
  - Designing workspace memory (~/.secureclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep SecureClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.secureclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.secureclaw/workspace` as canonical human-readable memory.
...
```
~/.secureclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in SecureClaw**, but keep a separable core library.

### Why integrate into SecureClaw?

- SecureClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `secureclaw memory recall "…" --k 25 --since 30d`
  - `secureclaw memory reflect --since 7d`
...
Pragmatic take for `~/.secureclaw/workspace`:
