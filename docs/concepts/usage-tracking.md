...
- `/usage cost` in chats: local cost summary aggregated from SecureClaw session logs.
- CLI: `secureclaw status --usage` prints a full per-provider breakdown.
- CLI: `secureclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
