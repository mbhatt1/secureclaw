...
read_when:
  - You want to understand what “context” means in SecureClaw
...
“Context” is **everything SecureClaw sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (SecureClaw-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How SecureClaw builds the system prompt

The system prompt is **SecureClaw-owned** and rebuilt each run. It includes:
...
By default, SecureClaw injects a fixed set of workspace files (if present):
