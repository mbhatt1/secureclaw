...
export async function handleWhatsAppStart(host: SecureClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: SecureClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: SecureClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: SecureClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: SecureClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: SecureClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: SecureClawApp): string | null {
...
function buildGatewayHttpHeaders(host: SecureClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: SecureClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: SecureClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: SecureClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: SecureClawApp) {
  const state = host.nostrProfileFormState;
