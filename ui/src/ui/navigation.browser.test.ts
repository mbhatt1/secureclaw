...
import { SecureClawApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = SecureClawApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("secureclaw-app") as SecureClawApp;
...
beforeEach(() => {
  SecureClawApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__SECURECLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  SecureClawApp.prototype.connect = originalConnect;
  window.__SECURECLAW_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/secureclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/secureclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/secureclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__SECURECLAW_CONTROL_UI_BASE_PATH__ = "/secureclaw";
    const app = mountApp("/secureclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/secureclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/secureclaw/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "secureclaw.control.settings.v1",
