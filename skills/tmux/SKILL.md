...
metadata:
  { "secureclaw": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---

# tmux Skill (SecureClaw)
...
```bash
SOCKET_DIR="${SECURECLAW_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/secureclaw-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/secureclaw.sock"
SESSION=secureclaw-python
...
- Use `SECURECLAW_TMUX_SOCKET_DIR` (legacy `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$SECURECLAW_TMUX_SOCKET_DIR/secureclaw.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `SECURECLAW_TMUX_SOCKET_DIR`).
