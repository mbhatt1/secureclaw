---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for SecureClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, SecureClaw cron scheduling for periodic checks, or version status checks on a machine running SecureClaw (laptop, workstation, Pi, VPS).
---

# SecureClaw Host Hardening

## Overview

Assess and harden the host running SecureClaw, then align it to a user-defined risk tolerance without breaking access. Use SecureClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim SecureClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. SecureClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if SecureClaw can access sensitive data.
...
### 2) Run SecureClaw security audits (read-only)

As part of the default read-only checks, run `secureclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `secureclaw security audit` (faster, non-probing)
2. `secureclaw security audit --json` (structured output)

Offer to apply SecureClaw safe defaults (numbered):

1. `secureclaw security audit --fix`

Be explicit that `--fix` only tightens SecureClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check SecureClaw version/update status (read-only)

As part of the default read-only checks, run `secureclaw update status`.
...
- Credential hygiene notes (location of SecureClaw creds, prefer disk encryption)
...
- SecureClaw security audit (re-run)
...
After SecureClaw install or first hardening pass, run at least one baseline audit and version check:

- `secureclaw security audit`
- `secureclaw security audit --deep`
- `secureclaw update status`

Ongoing monitoring is recommended. Use the SecureClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `secureclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `secureclaw update status`
...
Before creating, `secureclaw cron list` and match on exact `name`. If found, `secureclaw cron edit <id> ...`.
If not found, `secureclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `secureclaw update status` (preferred for source checkouts and channels)
2. `npm view secureclaw version` (published npm version)

## SecureClaw command accuracy

Use only supported commands and flags:

- `secureclaw security audit [--deep] [--fix] [--json]`
- `secureclaw status` / `secureclaw status --deep`
- `secureclaw health --json`
- `secureclaw update status`
- `secureclaw cron add|list|runs|run`

Do not invent CLI flags or imply SecureClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by SecureClaw compaction:
