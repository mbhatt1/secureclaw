...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> SecureClawNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> SecureClawNetworkStatusPayload {
        let status: SecureClawNetworkPathStatus = switch path.status {
        ...
        var interfaces: [SecureClawNetworkInterfaceType] = []
        if path.usesInterfaceType(.wifi) { interfaces.append(.wifi) }
        if path.usesInterfaceType(.cellular) { interfaces.append(.cellular) }
        if path.usesInterfaceType(.wiredEthernet) { interfaces.append(.wired) }
        if interfaces.isEmpty { interfaces.append(.other) }

        return SecureClawNetworkStatusPayload(
            status: status,
    ...
    private static func fallbackPayload() -> SecureClawNetworkStatusPayload {
        SecureClawNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
