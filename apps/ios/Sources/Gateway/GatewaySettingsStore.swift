...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.secureclaw.gateway"
    private static let nodeService = "ai.secureclaw.node"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.secureclaw.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.secureclaw.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent("secureclaw-gateway.log")
    }
