...
struct IOSGatewayChatTransport: SecureClawChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SecureClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(SecureClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> SecureClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(SecureClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [SecureClawChatAttachmentPayload]) async throws -> SecureClawChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [SecureClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(SecureClawChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(SecureClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SecureClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SecureClawChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: SecureClawAgentEventPayload.self)
                        {
