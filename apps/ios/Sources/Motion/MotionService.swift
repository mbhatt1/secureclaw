...
final class MotionService: MotionServicing {
    func activities(params: SecureClawMotionActivityParams) async throws -> SecureClawMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[SecureClawMotionActivityEntry], Error>) in
        ...
        return SecureClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: SecureClawPedometerParams) async throws -> SecureClawPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<SecureClawPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = SecureClawPedometerPayload(
