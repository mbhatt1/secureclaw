...
final class ContactsService: ContactsServicing {
    ...
    func search(params: SecureClawContactsSearchParams) async throws -> SecureClawContactsSearchPayload {
        ...
        return SecureClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: SecureClawContactsAddParams) async throws -> SecureClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return SecureClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return SecureClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> SecureClawContactPayload {
        SecureClawContactPayload(
