...
final class RemindersService: RemindersServicing {
    func list(params: SecureClawRemindersListParams) async throws -> SecureClawRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[SecureClawReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return SecureClawReminderPayload(
        ...
        return SecureClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: SecureClawRemindersAddParams) async throws -> SecureClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = SecureClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return SecureClawRemindersAddPayload(reminder: payload)
    }
