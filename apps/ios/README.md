...
This is an **alpha** iOS app that connects to an SecureClaw Gateway as a `role: node`.
...
3. On iOS: SecureClaw → Settings → Gateway → paste setup code → Connect
...
1. Select the `SecureClaw` scheme
...
cd apps/ios
xcodegen generate
xcodebuild test -project SecureClaw.xcodeproj -scheme SecureClaw -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/SecureClawKit` contains the shared transport/types used by the iOS app.
