...
private let chatUILogger = Logger(subsystem: "ai.secureclaw", category: "SecureClawChatUI")

@MainActor
@Observable
public final class SecureClawChatViewModel {
    public private(set) var messages: [SecureClawChatMessage] = []
    ...
    public var attachments: [SecureClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SecureClawChatPendingToolCall] = []
    public private(set) var sessions: [SecureClawChatSessionEntry] = []
    private let transport: any SecureClawChatTransport
    ...
    private var pendingToolCallsById: [String: SecureClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SecureClawChatTransport) {
    ...
    public var sessionChoices: [SecureClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [SecureClawChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [SecureClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SecureClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SecureClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SecureClawChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [SecureClawChatMessage]) -> [SecureClawChatMessage] {
        var result: [SecureClawChatMessage] = []
    ...
    private static func dedupeKey(for message: SecureClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SecureClawChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> SecureClawChatAttachmentPayload in
            SecureClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SecureClawChatSessionEntry {
        SecureClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SecureClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SecureClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SecureClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SecureClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> SecureClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
