...
import Foundation
import Testing
@testable import SecureClawChatUI
...
private final class TestChatTransport: @unchecked Sendable, SecureClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SecureClawChatHistoryPayload]
    private let sessionsResponses: [SecureClawChatSessionsListResponse]

    private let stream: AsyncStream<SecureClawChatTransportEvent>
    private let continuation: AsyncStream<SecureClawChatTransportEvent>.Continuation

    init(
        historyResponses: [SecureClawChatHistoryPayload],
        sessionsResponses: [SecureClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SecureClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SecureClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SecureClawChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [SecureClawChatAttachmentPayload]) async throws -> SecureClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return SecureClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SecureClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SecureClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SecureClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = SecureClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = SecureClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { SecureClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = SecureClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = SecureClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = SecureClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { SecureClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
