...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: SecureClawOAuthStore.AnthropicOAuthStatus = SecureClawOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: SecureClawOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = SecureClawOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = SecureClawOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try SecureClawOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. SecureClaw can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: SecureClawOAuthStore.AnthropicOAuthStatus,
