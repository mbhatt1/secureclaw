...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.secureclaw.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["SECURECLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "secureclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "secureclaw.nixMode") { return true }
