...
enum CommandResolver {
    private static let projectRootDefaultsKey = "secureclaw.gatewayProjectRootPath"
    private static let helperName = "secureclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let secureclawEntry = root.appendingPathComponent("secureclaw.mjs").path
        if FileManager().isReadableFile(atPath: secureclawEntry) { return secureclawEntry }
        let binEntry = root.appendingPathComponent("bin/secureclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/secureclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let secureclawPaths = self.secureclawManagedPaths(home: home)
        if !secureclawPaths.isEmpty {
            extras.insert(contentsOf: secureclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + secureclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func secureclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".secureclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func secureclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSecureClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("secureclaw.mjs").path,
            root.appendingPathComponent("bin/secureclaw.js").path,
        ]
    ...
    static func hasAnySecureClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.secureclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func secureclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let secureclawPath = self.projectSecureClawExecutable(projectRoot: root) {
                return [secureclawPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "secureclaw", subcommand] + extraArgs
            }
            if let secureclawPath = self.secureclawExecutable(searchPaths: searchPaths) {
                return [secureclawPath, subcommand] + extraArgs
            }

            let missingEntry = """
            secureclaw entrypoint missing (looked for dist/index.js or secureclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real secureclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/secureclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v secureclaw >/dev/null 2>&1; then
          CLI="$(command -v secureclaw)"
          secureclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/secureclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/secureclaw.mjs"
            node "$PRJ/secureclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/secureclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/secureclaw.js"
            node "$PRJ/bin/secureclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent secureclaw"
          pnpm --silent secureclaw \(quotedArgs);
        else
          echo "secureclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SecureClawConfigFile.loadDict()
