...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.secureclaw", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["SECURECLAW_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = SecureClawConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.secureclawExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "secureclaw CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["SECURECLAW_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "secureclaw@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "secureclaw@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "secureclaw@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "secureclaw@\(target)"])
            }

        statusHandler("Installing secureclaw@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed secureclaw@\(target)")
        } else {
