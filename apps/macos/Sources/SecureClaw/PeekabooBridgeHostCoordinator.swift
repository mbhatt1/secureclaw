...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.secureclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: SecureClawPeekabooBridgeServices?
    private static var secureclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("SecureClaw", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = SecureClawPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.secureclawSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.secureclawSocketPath, privacy: .public)")
    }
...
@MainActor
private final class SecureClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.secureclaw.peekaboo")
