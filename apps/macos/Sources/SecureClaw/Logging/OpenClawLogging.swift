...
enum SecureClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SecureClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SecureClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SecureClawLogging.bootstrapIfNeeded()
        let label = SecureClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SecureClawOSLogHandler: LogHandler {
...
struct SecureClawFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SecureClawLogging.parseLabel(self.label)
