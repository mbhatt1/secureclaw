...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (SecureClaw token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: SecureClawOAuthStore.AnthropicOAuthStatus = SecureClawOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.secureclaw", category: "anthropic-oauth")
...
enum SecureClawOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let secureclawOAuthDirEnv = "SECURECLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "SecureClaw OAuth token file not found"
            case .unreadableFile: "SecureClaw OAuth token file not readable"
            case .invalidJSON: "SecureClaw OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in SecureClaw OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "SecureClaw OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.secureclawOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".secureclaw", isDirectory: true)
