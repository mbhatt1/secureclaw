...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.secureclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SecureClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SecureClawSessionPreviewEntry,
    ...
    private static func cache(payload: SecureClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SecureClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SecureClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SecureClawKit.AnyCodable]) -> [SecureClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SecureClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SecureClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SecureClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SecureClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SecureClawChatMessage) -> String? {
