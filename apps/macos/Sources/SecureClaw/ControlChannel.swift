...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SecureClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.secureclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SecureClawKit.AnyCodable]()) {
                $0[$1.key] = SecureClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SecureClawProtocol.AnyCodable?) -> [String: SecureClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SecureClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SecureClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SecureClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("secureclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("secureclaw.control.agent")
}
