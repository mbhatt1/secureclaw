...
enum DebugActions {
    private static let verboseDefaultsKey = "secureclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "secureclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = SecureClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "SecureClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = SecureClawPaths.configURL
        guard
