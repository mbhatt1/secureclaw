...
private let webChatSwiftLogger = Logger(subsystem: "ai.secureclaw", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: SecureClawChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SecureClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(SecureClawChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SecureClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SecureClawGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    SecureClawChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    SecureClawAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SecureClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SecureClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SecureClawChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: SecureClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SecureClawChatView>) -> NSViewController
    {
