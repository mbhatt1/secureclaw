...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SECURECLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/secureclaw")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("secureclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("secureclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/secureclaw/ (rolling or stdout), or nil if none exist.
