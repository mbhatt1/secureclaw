...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.secureclaw", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.secureclaw", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [SecureClawChatMessage] = messages.compactMap { item in
