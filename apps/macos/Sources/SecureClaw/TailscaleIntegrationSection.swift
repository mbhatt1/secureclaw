...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.secureclaw/secureclaw.json. Prefer SECURECLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.secureclaw/secureclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.secureclaw/secureclaw.json. Restart the gateway to apply."
        }
