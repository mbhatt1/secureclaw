...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: SecureClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SecureClawProtocol.AnyCodable] = [
            "presence": .init([SecureClawProtocol.AnyCodable(entry)]),
        ]
        let payload = SecureClawProtocol.AnyCodable(payloadMap)
