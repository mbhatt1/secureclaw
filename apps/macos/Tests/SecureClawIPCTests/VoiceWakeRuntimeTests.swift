...
@Suite struct VoiceWakeRuntimeTests {
    @Test func trimsAfterTriggerKeepsPostSpeech() {
        let triggers = ["claude", "secureclaw"]
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["secureclaw"]
        let text = "hey secureclaw"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey secureclaw do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["secureclaw"], minPostTriggerGap: 0.3)
