...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("SECURECLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.SECURECLAW_STATE_DIR = path.join(base, ".secureclaw");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "secureclaw-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".secureclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
