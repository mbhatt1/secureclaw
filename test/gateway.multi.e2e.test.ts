...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `secureclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".secureclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          SECURECLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        SECURECLAW_GATEWAY_TOKEN: inst.gatewayToken,
        SECURECLAW_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SECURECLAW_GATEWAY_PORT: String(gwA.port),
          SECURECLAW_GATEWAY_TOKEN: gwA.gatewayToken,
          SECURECLAW_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          SECURECLAW_GATEWAY_PORT: String(gwB.port),
          SECURECLAW_GATEWAY_TOKEN: gwB.gatewayToken,
          SECURECLAW_GATEWAY_PASSWORD: "",
        }),
