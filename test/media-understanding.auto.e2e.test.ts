...
import type { SecureClawConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("secureclaw-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("secureclaw-bin-sherpa-");
      const modelDir = await makeTempDir("secureclaw-sherpa-model-");
      ...
      const cfg: SecureClawConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("secureclaw-bin-whispercpp-");
      const modelDir = await makeTempDir("secureclaw-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("secureclaw-bin-gemini-");
      ...
      const cfg: SecureClawConfig = { tools: { media: { image: {} } } };
