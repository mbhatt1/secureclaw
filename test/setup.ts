...
import { createTestRegistry } from "../src/test-helpers/channel-plugins.js";
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: SecureClawConfig) => {
    ...
    resolveAccount: (cfg: SecureClawConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: SecureClawConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
