 ...
 * `secureclaw update`. Use it to verify the completion installation flow
...
\`secureclaw update\`, \`secureclaw doctor\`, and \`secureclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# SecureClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`secureclaw completion --install\` later to enable.`));
    return;
  }
