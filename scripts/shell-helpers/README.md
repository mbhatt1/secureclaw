...
Inspired by Simon Willison's [Running SecureClaw in Docker](https://til.simonwillison.net/llms/secureclaw-docker).
...
On first command, ClawDock auto-detects your SecureClaw directory:

- Checks common paths (`~/secureclaw`, `~/workspace/secureclaw`, etc.)
...
| `clawdock-cli <command>`  | Run SecureClaw CLI commands                    |
...
| `clawdock-cd`        | Jump to the SecureClaw project directory  |
| `clawdock-config`    | Open the SecureClaw config directory      |
...
```bash
secureclaw channels login --channel whatsapp --verbose
```
...
2. Configure it in the SecureClaw config
...
- Bash or Zsh shell
- SecureClaw project (from `docker-setup.sh`)
