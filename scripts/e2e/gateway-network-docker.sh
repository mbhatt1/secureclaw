...
IMAGE_NAME="secureclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="secureclaw-net-e2e-$$"
GW_NAME="secureclaw-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "SECURECLAW_GATEWAY_TOKEN=$TOKEN" \
	  -e "SECURECLAW_SKIP_CHANNELS=1" \
	  -e "SECURECLAW_SKIP_GMAIL_WATCHER=1" \
	  -e "SECURECLAW_SKIP_CRON=1" \
	  -e "SECURECLAW_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
