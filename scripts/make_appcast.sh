...
ZIP=${1:?"Usage: $0 SecureClaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/secureclaw/secureclaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^SecureClaw-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/mbhatt1/secureclaw/releases/download/v${VERSION}/"}
