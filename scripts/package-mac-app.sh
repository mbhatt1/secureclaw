...
# Build and bundle SecureClaw into a minimal .app we can open.
# Outputs to dist/SecureClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/SecureClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="SecureClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.secureclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/secureclaw/secureclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/SecureClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SecureClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SecureClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/SecureClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/SecureClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/SecureClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/SecureClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/SecureClaw/Resources/SecureClaw.icns" "$APP_ROOT/Contents/Resources/SecureClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/SecureClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SecureClawKit resources"
SECURECLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SecureClawKit_SecureClawKit.bundle"
if [ -d "$SECURECLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SecureClawKit_SecureClawKit.bundle"
  cp -R "$SECURECLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SecureClawKit_SecureClawKit.bundle"
else
  echo "WARN: SecureClawKit resource bundle not found at $SECURECLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running SecureClaw"
killall -q SecureClaw 2>/dev/null || true
