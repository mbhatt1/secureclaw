...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/secureclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "SecureClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/secureclaw/scripts/mobile-reauth.sh'" --id secureclaw-auth
        ;;
    *)
