...
BASE_IMAGE="${BASE_IMAGE:-secureclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-secureclaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=secureclaw.sandbox=1)
NOTE
