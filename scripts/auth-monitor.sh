...
#
# Suggested cron: */30 * * * * /home/admin/secureclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send SecureClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., secureclaw-alerts)
...
STATE_FILE="$HOME/.secureclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via SecureClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use secureclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via SecureClaw to $NOTIFY_PHONE..."
            secureclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! SecureClaw is down. Run: ssh l36 '~/secureclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
