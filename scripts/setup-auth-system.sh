#!/bin/bash
# Setup SecureClaw Auth Management System
...
echo "=== SecureClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'secureclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  SecureClaw message: Send warning via SecureClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/secureclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/secureclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/secureclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now secureclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status secureclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
