...
const shouldBuild = () => {
  if (env.SECURECLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message) => {
  if (env.SECURECLAW_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[secureclaw] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["secureclaw.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
