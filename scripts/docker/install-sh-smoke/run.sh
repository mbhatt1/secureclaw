...
INSTALL_URL="${SECURECLAW_INSTALL_URL:-https://secureclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${SECURECLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${SECURECLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="secureclaw"
PACKAGE_NAME="${SECURECLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SECURECLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${SECURECLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${SECURECLAW_INSTALL_LATEST_OUT:-}"
fi
