...
show_usage() {
    cat << EOF
clawlog - SecureClaw Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View SecureClaw logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    SecureClaw logs flow through the macOS unified log (subsystem: ai.secureclaw).
