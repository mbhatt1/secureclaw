# SecureClaw ACP Bridge

This document describes how the SecureClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`secureclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
SecureClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a SecureClaw Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `secureclaw acp` over stdio.

Example config:

```bash
secureclaw config set gateway.remote.url wss://gateway-host:18789
secureclaw config set gateway.remote.token <token>
```

Example run:

```bash
secureclaw acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
secureclaw acp --session agent:main:main
secureclaw acp --session agent:design:main
secureclaw acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “SecureClaw ACP” to start a thread.

## Execution Model

- ACP client spawns `secureclaw acp` and speaks ACP messages over stdio.
...
```bash
secureclaw acp --session agent:main:main
secureclaw acp --session-label "support inbox"
secureclaw acp --reset-session
```
...
`secureclaw acp` resolves the Gateway URL and auth from CLI flags or config:
